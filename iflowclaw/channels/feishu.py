...
class FeishuChannel(Channel):
    ...
    async def send_message(self, jid: str, text: str) -> None:
        ...
        req = self._lark.im.v1.CreateMessageRequest.builder().receive_id_type("chat_id").request_body(
    ...
    def _detect_ws_client(self) -> Any | None:
        ...
        try:
            return client_cls.builder().app_id(self._config.feishu_app_id).app_secret(self._config.feishu_app_secret).on_event(on_event).build()
        except Exception:
            return None
    ...
    async def _handle_event(self, data: Any) -> None:
        try:
            ...
            create_time = message.get("create_time") if isinstance(message, dict) else getattr(message, "create_time", None)
            timestamp = (
                datetime.fromtimestamp(int(create_time) / 1000, tz=timezone.utc).isoformat().replace("+00:00", "Z")
                if create_time
            ...
            self._on_chat_metadata(chat_jid, datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"), None, "feishu", True)
            ...
            message_type = message.get("message_type") if isinstance(message, dict) else getattr(message, "message_type", "text")
