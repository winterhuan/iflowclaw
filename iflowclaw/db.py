...
from .types import AgentConfig, ChatInfo, NewMessage, RegisteredGroup, ScheduledTask, TaskRunLog
...
def set_registered_group(jid: str, group: RegisteredGroup) -> None:
    ...
    with _lock:
        conn.execute(
            """
            INSERT INTO registered_groups (jid, name, folder, trigger_pattern, added_at, agent_config, requires_trigger, is_main)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(jid) DO UPDATE SET
              name = excluded.name,
