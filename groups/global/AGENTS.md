...
**每个群组拥有独立的专属工作目录**，路径为 `groups/<folder>/`（如 `groups/content-team/`、`groups/dev-team/`）。

- 当前工作目录指向你**所在群组的专属目录**（读写权限）
- 使用相对路径保存文件时，是相对于你的群组目录，如 `conversations/`、`notes.md` 等
- 不同群组的文件相互隔离，不能跨群访问其他群组的私有文件

## Skills 存放策略

| 层级 | 路径 | 用途 |
|------|------|------|
| 项目 | `项目根目录/.iflow/skills/` | 本项目所有群组共享 |
| 群组 | `groups/<folder>/.iflow/skills/` | 群组专属 |
...
save_memory(
  key: "project_goal",
  value: "构建一个电商网站，使用 React + Node.js",
  category: "context",
  importance: 4
)
```
